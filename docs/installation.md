...
The installer uses `requirements.txt` to install `jsonschema` and `PyYAML` for local contract validation. By default it also runs `vela local-env install`, which:
...
VELA provides `.codex/config.toml.example`, `AGENTS.md` templates, and MCP/profile templates. It does not silently rewrite your global Codex configuration. To preview or apply a profile after installation, use `envctl apply-profile <profile> --dry-run` first, then `--commit` only when the diff is acceptable.
