...
内部稳定标识仍然使用 `agent_id`，但对外说明、dispatch 附加字段和编排结果应同时给出 `display_name`，格式优先写成：
...
- `<LOCAL_ENV_ROOT>\skills\catalog\research_team_playbooks.json`
- `<LOCAL_ENV_ROOT>\skills\catalog\control_kernel.json`
...
- `<LOCAL_ENV_ROOT>\skills\schemas\agent_dispatch_card.schema.json`
- `<LOCAL_ENV_ROOT>\skills\schemas\project_agent_definition.schema.json`
- `<LOCAL_ENV_ROOT>\skills\scripts\validate_agents_contract.py`
- `<LOCAL_ENV_ROOT>\skills\scripts\plan_research_team.py`
- `<LOCAL_ENV_ROOT>\skills\scripts\bootstrap_agent_dispatch.py`
