...
This skill is the local research workflow wrapper. It does not fork, simplify, or rewrite guizang-ppt-skill's visual system. When making HTML web decks, first activate and follow the installed original `guizang-ppt-skill`; this wrapper only adds research-route confirmation, evidence boundaries, output-mode decisions, and final QA.
