...
CONTRACTS = {
    "control_kernel": (
        CATALOG_ROOT / "control_kernel.json",
        SCHEMAS_ROOT / "cybernetic_control_kernel.v1.schema.json",
    ),
    ...
    "cybernetic_source_rule_crosswalk": (
        CATALOG_ROOT / "cybernetic_source_rule_crosswalk.json",
        SCHEMAS_ROOT / "cybernetic_source_rule_crosswalk.v1.schema.json",
    ),
    "subagent_registry": (
...
READING_EVIDENCE_PATH = CATALOG_ROOT / "engineering_cybernetics_source_evidence.json"

MEMORY_DECISIONS = {"control_kernel", "skill", "obsidian", "codex_native", "discard"}
CORE_CONTROLLERS = {"research-autopilot", "research-stack-manager", "project-retrospective-evolver", "envctl"}
...
def validate_cybernetics_contracts() -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    errors.extend(_collect_source_rule_crosswalk_errors(payloads.get("cybernetic_source_rule_crosswalk", {})))
    ...
    return build_validator_result(
        validator="validate_cybernetics",
        scope="cybernetic_control_kernel",
        errors=errors,
        warnings=warnings,
        details=details,
    )
...
def _collect_kernel_link_errors(payloads: dict[str, dict[str, Any]]) -> list[str]:
    kernel = payloads.get("control_kernel", {})
    errors: list[str] = []
    link_map = {
    ...
    for key, expected in link_map.items():
        if not expected.exists():
            errors.append(f"control_kernel:{key}:missing-target:{expected}")
        elif kernel.get(key) != str(expected.relative_to(SKILLS_ROOT.parent)).replace("\\", "/"):
            errors.append(f"control_kernel:{key}:unexpected-ref:{kernel.get(key)}")
    ...
    errors.extend(f"control_kernel:missing-controller:{item}" for item in sorted(missing_controllers))
    return errors
...
def _collect_source_rule_crosswalk_errors(crosswalk: dict[str, Any]) -> list[str]:
    ...
    for source_id, count in Counter(source_ids).items():
        if count > 1:
            errors.append(f"cybernetic_source_rule_crosswalk:duplicate-source:{source_id}")
    for principle_id, count in Counter(principle_ids).items():
        if count > 1:
            errors.append(f"cybernetic_source_rule_crosswalk:duplicate-principle:{principle_id}")
    required_principles = {
    ...
    errors.extend(
        f"cybernetic_source_rule_crosswalk:missing-principle:{item}"
    ...
    for principle in principles:
        ...
        errors.extend(
            f"cybernetic_source_rule_crosswalk:{principle_id}:unknown-source:{item}"
...
def build_skill_audit_report() -> dict[str, Any]:
    ...
    kernel = load_json(CATALOG_ROOT / "control_kernel.json")
    active_skills = {
        name: item
    ...
    return {
        "schema_version": "cybernetics_skill_audit.v1",
        ...
        "suggested_next_checks": [
            ...
            "python -m skills.scripts.envctl cybernetics evolution-backlog",
...
def write_report(report: dict[str, Any], output: Path | None = None) -> Path:
    if output is None:
        ...
        output = OUTPUTS_ROOT / "reports" / "cybernetics-daily" / f"{date}.json"
