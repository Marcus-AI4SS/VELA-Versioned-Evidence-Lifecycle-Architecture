...
必要时把 `<VELA_RUNTIME_ROOT>\skills\scripts\plan_research_team.py`、`bootstrap_agent_dispatch.py` 和 `validate_vela_runtime.py` 视为稳定的运行逻辑参考。
...
2. 列出候选 route，并判断是否属于新链条、路线切换或语义模糊

3. 如属于新链条或路线切换，先输出路线确认卡并等待用户确认；如语义模糊，先提出 1-3 个引导式问题

4. 选择 route

5. 判断 `project_scope_class`

6. 生成 clarification card；若需要路线确认，必须写入 `route_confirmation_required`、`route_confirmation_question` 和 `user_confirmed_route`

7. 判断是否进入多 agent planning

8. 选择 profile

9. 选择 leaf skills

10. 判断是否需要 helper skills

11. 判断是否需要本地 / 云端路由

12. 输出说明卡

13. 如果是项目型任务，先把多 agent 编排交给 `research-team-orchestrator`

14. 再输出 `Agent Dispatch Card`
...
该 route 默认进入多 agent planning，并按需组合：



工具层只作为 helper：

- `Figma` / `figma-dev-mode-mcp` 只提供设计上下文，不直接决定实现。

- `playwright-mcp` / `chrome-devtools` / `agent-browser` 只服务交互验证和浏览器壳调试。

- `Build macOS Apps` 只服务 macOS 原生 app，不接管 Windows 主开发链。
...
- Python / Git / PowerShell / Codex config / MCP profile 维护 -> `environment-ops` 路线，仍由 `vela-runtime-manager` 执行，不直接散落成临时 shell 修补
...
- 社媒可复用抓取 -> reusable agent-browser templates 只作为浏览器证据模板，不取代 `social-platform-reader` 这个用户侧编排 skill
...
- 项目文件夹清理、整理、归档、交接、收尾前目录收束、死文件检查、临时文件清理或大量生成产物后的组织化 -> `project-folder-hygiene`。先确认项目根目录和写权限，再分类为保留、可删、需确认、需归档；不得把项目清理混同于VELA 运行时治理，也不得静默删除 tracked 文件、用户材料、证据、PDF、数据、notebook、Zotero/Obsidian 内容、日志或项目合同。

- 社会科学论文送审包、A/B 稿融合、公开/受限材料同步和复现交付冻结 -> `social-science-submission-packager`，再调用图表、复现、审稿和导出子技能
...
- 论文写作、写论文、选题、没有新意、理论和文本脱节、章节逻辑、脚注格式、论文修改、重写、润色、中英转换、缩写、扩写、去 AI 味、降 AIGC、标题、摘要、引言、文献综述、方法、结果、讨论或结论 -> `writing-export` route 和 `manuscript-writing-studio`。先读取 `manuscript_writing_workflow.json` 并声明语言目标、学科目标和输出容器：LaTeX、Word、DOCX 或纯文本。英文期刊可更多借鉴 Nature-style；中文期刊以本地中文社科规则为主；计算社科和方法技术型论文可借鉴技术论文结构；传统量化和质性研究以本地社科逻辑为主。人文社科论文先确认论文类型、研究对象、核心问题、论点方向、理论工具、材料边界和预期长度；理论必须落到材料，引文之后必须分析，章节必须递进。去 AI 味或降 AIGC 明确信号出现时，锁定事实、引用、结果和边界后调用 `academic-humanization-studio` 做 detect/rewrite/second-pass audit；涉及引用或证据性断言变化时，仍必须回到 `writing-reference-capture` 和 citation gate。
...
- 审稿工作流 -> 必须参考 `peer_review_workflow.json`。外部仓库的优点只作为可审查模式采用：专项问题账本、领域化 reviewer card、full-text reading gate、独立 subtle logic flaw audit、reviewer independence、response tracker 和 action mapping；不得整包安装外部审稿系统，不得让 reviewer 直接改主产物。

- 项目收尾复盘、对话流总结、技能演化判断和 Obsidian 沉淀 -> `project-retrospective-evolver -> vela-runtime-manager / obsidian-research-sync`

- `local-cloud-router` 是 helper，不是用户主入口

- `skill-vetter` 只在第三方组件取舍时触发

- CNKI 批量下载到 Zotero -> 先走受控链路：默认用 `envctl cnki-zotero find/fetch` 和可控 Chrome/CDP 做发现与下载编排；`cnki-mcp` 只在实时冒烟测试通过后作为临时候选传感器。浏览器或用户批量下载脚本只做已授权下载，`envctl cnki-zotero ingest-plan` 生成导入计划，Zotero 写入必须显式执行。项目任务必须下载到项目自己的 `outputs/inbox/cnki-downloads`；不得保存 cookies，不得绕过登录、验证码或访问权限。遇到滑块或安全验证时写入 `captcha_checkpoint`，用户在可见浏览器手动完成验证后，从报告里的 `resume_candidates` 继续。
...
6. 外部 AER/StatsPAI/Full Empirical Analysis 的优点只作为VELA 契约采用：识别优先、现代 DID/IV/RDD/shift-share/SCM 诊断、Stata do-file 生态、稳健性矩阵、复现包纪律和结果解释边界。不得把外部 DSL 或自动包当作无需审计的黑箱。

7. 如果用户要求“降 AIGC”或“去 AI 味”，只允许在结论、结果、变量、估计值、引用和局限已经锁定后调用 `academic-humanization-studio` 做表层审计；不得为了像人工写作而改变事实、数据、模型输出或引用，也不得承诺检测绕过。
...
2. 先运行或参考 `scan_vela_runtime.py` 与 `validate_vela_runtime.py`。
...
3. 先形成 figure brief：图的用途、论文主张、数据或概念依据、面板计划、全部文字标签、连接线含义、配色、输出格式和风险检查。

4. 对 Excel、CSV、模型输出或真实数据图，先读取 `scientific_figure_workflow.json`：正式图默认先做数据体检，保留 process_data，按中英文字体 fallback、字号、图幅、DPI、统计标注和图注规则执行。

5. 不得因为科研图或论文图而拒绝 image2。机制图、概念图、流程图、研究设计图和非数值科研插图可以用 image2；精确数据图、回归图、事件研究图和系数图先用代码或可编辑工具控制数据。

6. 调用 image2 前必须锁定结构和文字；不得让 image2 编造数字、坐标、p 值、样本量、DOI、来源或因果关系。

7. 图像输出后必须检查文字可读性、箭头关系、面板结构、黑白可读性、数据真实性、字体字号和图注证据链；未通过则修图，不得把状态写成完成。

8. 如果是投稿包或完整论文图表组，`social-science-submission-packager` 仍是总控，`research-figure-studio` 和 `figure-table-studio` 是图表子链。
...
7. 用户在 Chrome 中登录 CNKI 后，优先让 Codex 通过 agent-browser 或直接 Chrome DevTools Protocol 接入这个已登录 Chrome：
...
10. 如果普通 Chrome 因默认资料目录限制无法提供可控下载目录，启动专用 CNKI 可控窗口：`powershell -ExecutionPolicy Bypass -File skills\scripts\open-cnki-controlled-chrome.ps1`。用户只需在该窗口登录 CNKI 和处理验证码；随后用 `browser-download --direct-cdp --cdp 9333` 或 `fetch --cdp 9333` 下载到项目收件箱。
...
12. 如果可控浏览器不可用，才退回用户已安装的批量下载脚本；下载目标仍必须是项目收件箱。Codex 不保存 cookies 到仓库，不自动拖动验证码，不绕过访问限制。
