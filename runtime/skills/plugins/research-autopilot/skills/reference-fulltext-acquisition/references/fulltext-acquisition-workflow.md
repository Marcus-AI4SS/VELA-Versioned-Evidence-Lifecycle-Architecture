...
2. 只用脚本 HTTP 请求下载 Scholar 右侧链接。Academia、ResearchGate、SSRN、机构库和出版社页面经常对脚本返回 `HTTP 403`、HTML 登录页或非 PDF，但在官方 Chrome 插件接管的用户已登录 Chrome 中可以打开 PDF 阅读器或生成最终签名 PDF URL。
...
补救顺序：先 DOI 精确检索并点击右侧 PDF；能用官方 Chrome 插件或 Computer Use 时优先在用户 Chrome 中完成点击和下载；打开候选后记录最终 URL 或点击浏览器下载按钮；扫描下载目录并标准化命名；自动校验失败但首页疑似正确的 PDF 进入双独立审核；错链保留 rejected 证据并写明拒收理由。
...
Windows 下 `agent-browser.cmd` 可能错误处理 URL 中的 `%22/%2C` 编码；只有原生 Chrome、Computer Use、可控 Chrome/CDP 都不足时，才考虑直接调用 `agent-browser-win32-x64.exe`，并确保参数未被 shell 展开。

### 4.0 用户 Chrome 优先规则：官方 Chrome / Computer Use

当脚本请求失败但用户能在自己的已登录 Chrome 中看到右侧 PDF 时，不要继续把该条记为“不可获取”。Google Scholar、Academia、ResearchGate、SSRN、机构库 PDF viewer、Google Scholar Reader、浏览器 PDF 下载按钮和下载目录回收，默认优先使用官方 Chrome 插件接管用户 Chrome；需要可视点击、文件对话框或下载目录确认时使用 Computer Use。只有原生能力不可用、未授权或无法连接时，才退回隔离可控浏览器、无头浏览器或脚本请求。

1. 使用官方 Chrome 插件接管用户已登录 Chrome 打开 Scholar 查询页；若插件、账号、easyScholar、Google Scholar Reader 或下载记录只存在于用户浏览器，不能用隔离浏览器替代最终判断。
...
使用官方 Chrome 插件、Computer Use、可见 Chrome/CDP 或其他可审计浏览器会话，避免无头脚本制造大量不可解释失败。每篇文献至少记录：
...
### 5.0 CNKI 官方 Chrome / Computer Use 优先规则

CNKI 下载依赖用户的授权登录态、机构权限、下载插件/CAJ 阅读器关联和验证码处理。涉及这些环节时，默认优先使用官方 Chrome 插件接管用户已登录 Chrome；需要可视点击、文件对话框、扩展弹窗或下载目录确认时使用 Computer Use。专用可控 Chrome/CDP 或 `envctl` 是批量执行器，不替代用户浏览器登录态。

1. 先在用户已登录 Chrome 中确认 CNKI 可访问、账号/机构权限有效、下载按钮可见。
2. 检索、详情页核验、下载按钮点击、CAJ/PDF 文件落地和验证码恢复，优先在用户 Chrome 中完成或由原生工具接管该会话完成。
3. 只有原生 Chrome/Computer Use 不足、用户未登录、用户明确要求隔离环境，或需要稳定批量下载、项目下载目录控制、断点续跑时，才启动专用可控 Chrome/CDP。
4. 不保存、导出或复用用户 cookies；不绕过登录、机构权限、滑块、短信、图片验证码或安全验证。
5. 浏览器下载成功但脚本状态失败时，扫描用户下载目录和项目收件箱，按题名、作者、来源、DOI/刊期或 CNKI 元数据侧车文件回收并重命名。

### 5.1 CNKI 执行顺序

1. 默认先用官方 Chrome 插件接管用户 Chrome，或用 `envctl cnki-zotero find/fetch` 做候选发现。
2. 用官方 Chrome 插件、Computer Use、可控 Chrome/CDP 或用户已安装的批量下载脚本执行授权下载。
