    ...
    *,
    ...
    return {
        ...
        "next_steps": [
            ...
            "Use the official Zotero plugin or local Zotero connector workflow to add verified local PDFs or DOI records after reviewing the import plan.",
        ],
    }
...
def build_inbox_audit(
    *,
    ...
    return {
        ...
        "by_action": dict(sorted(Counter(item["action"] for item in files).items())),
        "files": files,
        "zotero_import_plan": build_zotero_import_plan(files),
    }
    ...
    for item in files:
        if item["action"] == "local_pdf_add_from_file":
            plan.append(
                {
                    "file": item["path"],
                    "tool": "official_zotero_plugin_or_local_connector_add_from_file",
            ...
            plan.append(
                {
                    ...
                    "tool": "manual_review",
                    "requires_review": True,
                    "reason": "CAJ is not a default automatic Zotero import target. Attach manually or convert through a trusted user-approved path.",
                }
            )
    return plan
