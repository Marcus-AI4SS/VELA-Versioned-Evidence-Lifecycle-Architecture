...
def validate_environment_layer_contract() -> dict[str, Any]:
    ...
    details = {
        ...
        "local_memory_contract_enabled": _local_memory_contract_enabled(memory),
...
def _collect_memory_contract_errors(contract: dict[str, Any], memory: dict[str, Any]) -> list[str]:
    ...
    if mem.get("decision") != "local_contract_with_optional_codegraph":
        errors.append(f"environment_layers:memory-decision-not-local-contract:{mem.get('decision')}")
    ...
    if runtime.get("selected_adapter") != "local_contract_only":
        errors.append(f"environment_layers:runtime-adapter-not-local-contract:{runtime.get('selected_adapter')}")
...
def _collect_governance_contract_errors(
    ...
    for item in [
        ...
        "runtime memory candidates must be scored, reviewed, and routed before becoming durable rules",
...
def _local_memory_contract_enabled(memory: dict[str, Any]) -> bool:
    runtime = memory.get("runtime_adapter_policy", {})
    return runtime.get("selected_adapter") == "local_contract_only" and runtime.get("status") == "enabled"
