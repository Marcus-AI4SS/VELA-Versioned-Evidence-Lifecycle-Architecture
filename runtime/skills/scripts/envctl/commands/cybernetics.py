...
try:
    ...
    from envctl.cybernetics import (
        build_evolution_backlog_summary,
        build_reading_status,
        build_skill_audit_report,
        write_report,
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.action == "evolution-backlog":
        ...
        return 0
    raise ValueError(f"unsupported cybernetics action: {args.action}")


def add_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser("cybernetics")
    action = parser.add_subparsers(dest="action", required=True)

    audit = action.add_parser("audit-skills")
    audit.add_argument(
        "--output",
        help="Write the generated audit report to a path, or use 'auto' for skills/outputs/reports/cybernetics-daily/YYYY-MM-DD.json.",
    )
    audit.add_argument(
        "--dry-run",
        action="store_true",
