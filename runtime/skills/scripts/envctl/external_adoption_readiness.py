...
def _probe_vela() -> dict[str, Any]:
    ...
    legacy_manifest = vela_root / "research-stack" / "local-environment" / "manifest.json"
    runtime_manifest = vela_root / "runtime" / "manifest.json"
    manifest = legacy_manifest if legacy_manifest.exists() else runtime_manifest
    return {
...
def validate_external_adoption_readiness() -> dict[str, Any]:
    ...
    for upstream, probe in probes.items():
        if probe.get("ok") is not True:
            if _review_requires_runtime_probe(index.get(upstream, {})):
                errors.append(f"adoption-readiness:runtime-probe-failed:{upstream}")
            else:
                warnings.append(f"adoption-readiness:optional-runtime-probe-unavailable:{upstream}")
    ...
    details = {
        ...
        "watch_only_entries": [
            {
                "upstream": "external-memory-service-patterns",
                "reason": "External memory services are not default Codex runtime dependencies. Local contracts remain the memory source of truth; no installed runtime is claimed.",
            }
        ],
...
def _review_requires_runtime_probe(review: dict[str, Any]) -> bool:
    status = str(review.get("current_status", "")).lower()
    ...
    if decision == "watch" or any(marker in status for marker in optional_markers):
        return False
    runtime_markers = ("installed", "active plugin", "enabled", "runtime")
    return any(marker in status for marker in runtime_markers)
