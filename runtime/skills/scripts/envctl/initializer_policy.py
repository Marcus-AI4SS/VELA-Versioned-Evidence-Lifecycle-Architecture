...
PRIVATE_STRING_PATTERNS = [
    ...
    re.compile(r"<AI_ENV_ROOT>", re.IGNORECASE),
    re.compile(r"private[-_ ]?desktop", re.IGNORECASE),
    re.compile(r"generated[-_ ]?desktop[-_ ]?shell", re.IGNORECASE),
    re.compile(r"<OBSIDIAN_VAULT>", re.IGNORECASE),
    re.compile(r"Zotero", re.IGNORECASE),
]
GUI_ONLY_AGENT_PREFIXES = ("app-", "desktop-app-")
GUI_RELEASE_PATH_MARKERS = ("release-package", "desktop-build", "app-release")
...
def _validate_public_initializer(local_manifest: dict[str, Any], vela_manifest: dict[str, Any]) -> list[str]:
    ...
    for file_name, payload in vela_manifest.get("project_agents", {}).items():
        ...
        if isinstance(agent_id, str) and agent_id.startswith(GUI_ONLY_AGENT_PREFIXES):
    ...
    for path, value in _walk_strings(vela_manifest):
        ...
        if any(marker in lowered for marker in GUI_RELEASE_PATH_MARKERS):
