...
def _collect_contract_errors(contract: dict[str, Any]) -> list[str]:
    ...
    errors.extend(_collect_style_preset_contract_errors(contract.get("style_preset_contract", {})))
...
def _collect_style_preset_contract_errors(style_contract: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    expected = {
    ...
    for key, value in expected.items():
        if style_contract.get(key) != value:
            errors.append(f"scientific-figure-workflow:style-preset-contract-mismatch:{key}:{style_contract.get(key)}")
    role = style_contract.get("external_visual_source_role", "")
    if "not an independent runtime entrypoint" not in role:
        errors.append("scientific-figure-workflow:nature-visual-source-role-missing")
    return errors
