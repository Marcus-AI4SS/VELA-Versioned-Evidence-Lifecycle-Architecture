...
def resolve_external_repo_root(
    ...
    *,
    ...
    if isinstance(repos, dict):
        ...
        if isinstance(settings_value, str) and settings_value.strip() and "<" not in settings_value:
            return _configured_path(settings_value)

    workspace_root = REPO_ROOT.parent.parent if (REPO_ROOT.parent / ".git").exists() else REPO_ROOT.parent
    sibling = (workspace_root / sibling_name).resolve()
    if sibling.exists():
