...
PLUGIN_ID_OVERRIDES = {
    ...
    "research-autopilot": "research-autopilot@vela-runtime-local",
...
def render_markdown(payload: dict) -> str:
    ...
    lines = [
        ...
        "# VELA Runtime Proposal Baseline",
    ...
    lines.extend(
        ...
        [
            ...
            "- 检查总入口是否仍然只剩 `research-autopilot`；`project-retrospective-evolver`、`vela-runtime-manager` 只能是路线内控制器。",
...
def main() -> None:
    ...
    contract_assets = {
        ...
        "governance_source_rule_crosswalk_schema": (ROOT / "schemas" / "governance_source_rule_crosswalk.v1.schema.json").exists(),

        "project_initializer_manifest": (ROOT / "catalog" / "project_initializer_manifest.json").exists(),

        "governance_source_rule_crosswalk": (ROOT / "catalog" / "governance_source_rule_crosswalk.json").exists(),
