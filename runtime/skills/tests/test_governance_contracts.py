...
from scripts.envctl.governance import (
...
class GovernanceContractTests(unittest.TestCase):
    ...
    def test_governance_contracts_use_validator_envelope(self) -> None:

        result = validate_governance_contracts()

        schema = load_json(ROOT / "schemas" / "validator_result.schema.json")

        self.assertEqual(collect_schema_errors(result, schema, "governance_result"), [])

        self.assertTrue(result["ok"])

        self.assertEqual(result["validator"], "validate_governance")

        self.assertEqual(result["warnings"], [])

        self.assertNotIn("reading_status", result["details"])

        self.assertNotIn("workflow_governance_reading_passes", CONTRACTS)
    ...
    def test_source_rule_crosswalk_covers_recent_review_gaps(self) -> None:
        ...
        crosswalk = load_json(ROOT / "catalog" / "governance_source_rule_crosswalk.json")

        schema = load_json(ROOT / "schemas" / "governance_source_rule_crosswalk.v1.schema.json")
        ...
        self.assertEqual(
...

            {
                ...
                "public-platform-harness-governance",

                "public-platform-ai-code-review-governance",
    ...
    def test_reading_status_truthfully_tracks_ocr_and_unfinished_passes(self) -> None:
        ...
        english = next(item for item in status["sources"] if item["id"] == "workflow-governance-en")
    ...
    def test_evolution_backlog_records_governance_kernel_bootstrap(self) -> None:

        summary = build_evolution_backlog_summary()

        self.assertGreaterEqual(summary["event_count"], 6)

        self.assertGreaterEqual(summary["by_target"].get("governance_kernel", 0), 1)
