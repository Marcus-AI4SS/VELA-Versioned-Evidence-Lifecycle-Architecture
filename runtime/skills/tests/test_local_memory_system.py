...
class LocalMemorySystemTests(unittest.TestCase):
    ...
    def test_policy_adopts_memory_repos_with_governed_runtime_adapter(self) -> None:
        ...
        self.assertIn("rohitg00/agentmemory", repos)
        ...
        self.assertEqual(policy["runtime_adapter_policy"]["selected_adapter"], "agentmemory")
    ...
    def test_reconciliation_report_is_schema_valid_and_does_not_write_source(self) -> None:
        ...
        report = build_memory_reconciliation_report(probe_agentmemory=False)

        self.assertEqual(validate_memory_reconciliation_report(report), [])

        self.assertFalse(report["source_files_written"])

        self.assertEqual(report["agentmemory"]["configured"], True)

        self.assertEqual(report["agentmemory"]["checked"], False)
    ...
    def test_retrieval_interfaces_have_structured_governance_controls(self) -> None:
        ...
        delete_text = " ".join(
            ...
            controls["agentmemory_governance_delete"]["required_fields"]

            + controls["agentmemory_governance_delete"]["safety_rules"]

        )

        self.assertIn("user_confirmation", delete_text)

        self.assertIn("source rules", delete_text)
    ...
    def test_secret_signal_is_hard_blocked_even_if_repeated(self) -> None:
        ...
        candidate = candidate_from_text(
            ...
            proposed_target="governance_kernel",
