...
class ManuscriptWritingWorkflowTests(unittest.TestCase):
    ...
    def test_language_and_discipline_targets_are_explicit(self) -> None:
        ...
        self.assertIn("hardikpandya/stop-slop", source_refs)
        self.assertIn(
            "structural scan for binary contrast, negative listing, rhetorical setup, false agency, and rhythm monotony",
            source_refs["hardikpandya/stop-slop"]["adopted_as"],
        )
    ...
    def test_publication_and_writing_rules_reference_manuscript_studio(self) -> None:
        ...
        self.assertIn("no_stop_slop_overreach", safety_ids)
        self.assertIn("no_defensive_argument_drift", safety_ids)
        self.assertIn("no_english_to_chinese_term_guessing", safety_ids)
        self.assertIn("no_uniform_fragmented_paragraphs", safety_ids)
        ...
        self.assertIn("direct_argument_progression_checked", check_ids)
        self.assertIn("four_sentence_storyline_checked", check_ids)
        self.assertIn("method_transition_continuity_checked", check_ids)
        self.assertIn("contribution_posture_checked", check_ids)
        self.assertIn("rhythm_variety_checked", check_ids)
        self.assertIn("reader_facing_terms_checked", check_ids)
    ...
    def test_route_and_runtime_sync_include_manuscript_studio(self) -> None:
        ...
        self.assertIn("stop slop", route["keywords"])
        ...
        self.assertIn("Stop Slop Adapter", skill_text)
        ...
        self.assertIn("四句话概括", skill_text)
        self.assertIn("不能先查英文再自行翻译成中文术语", skill_text)
        self.assertIn("不能把正文切成一串四五行小碎段", skill_text)
        ...
        self.assertIn("Stop Slop Adapter", humanization_text)
        self.assertIn("Do not promise to bypass", humanization_text)
        self.assertIn("uniform four-to-five-line paragraph blocks", humanization_text)
        self.assertIn("Do not add extra explanation just to avoid a watched phrase", humanization_text)
        self.assertIn("Chinese-language scholarship", humanization_text)
        ...
        self.assertIn("Writing Quality Freeze", packager_text)
        self.assertIn("Chinese-language scholarship", packager_text)
